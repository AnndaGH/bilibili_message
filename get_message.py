...
def get_message(talker_id):
    url = 'https://api.vc.bilibili.com/svr_sync/v1/svr_sync/fetch_session_msgs?sender_device_id=1&talker_id={}&session_type=1&size=50&begin_seqno=0&build=0&mobi_app=web'.format(talker_id)
    ...
    r = requests.get(url, headers=head, cookies=cookies)
    r.encoding = 'utf-8'
    msgs = json.loads(r.text)
    for msg in reversed(msgs['data']['messages']):
        try:
            print(msg['timestamp'], json.loads(msg['content'])['content'])
        except:
            pass
    return

def get_talker_id(cookies):
    ...
    for id in content['data']['session_list']:
        get_message(id['talker_id'])


if __name__ == '__main__':
    # Get cookie
    domain = '.bilibili.com'
    cookies = get_cookies_from_chrome(domain)
    # Get talker id
    get_talker_id(cookies)
